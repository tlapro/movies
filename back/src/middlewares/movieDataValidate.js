validateMovie = async (req, res, next) => {
    try {
        ...
        if (typeof title !== 'string' || title.trim() === '') {
            return res.status(400).json({ error: "El título es erróneo o no se ha completado" });
        }
            
        if (isNaN(year) || year < 1921 || year >= 2025) {
            return res.status(400).json({ error: "El año debe ser mayor a 1921 y menor o igual al actual" });
        }
    
        if (!directorRegex.test(director)) {
            return res.status(400).json({ error: "El director debe completarse con nombre y apellido." });
        }
        if (!Array.isArray(genre) || genre.length === 0) {
            return res.status(400).json({ error: "Ningún género recibido" });
        }
    
        if (!timePattern.test(duration)) {
            return res.status(400).json({ error: "La hora debe tener formato 'Xh XXmin'." });
        }
    
        if (!validRate.test(rate)) {
            return res.status(400).json({ error: "El rate debe tener el formato X.X/X.XX." });
        }        
